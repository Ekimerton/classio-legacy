...
def search(university):
    ...
    if university in ['queens', 'waterloo']:
        form = ClassForm()
        if form.validate_on_submit():
            lunch_time = utils.pad_hour(form.lunch_start.data.hour) + utils.pad_hour(form.lunch_start.data.minute) + utils.pad_hour(form.lunch_end.data.hour) + utils.pad_hour(form.lunch_end.data.minute)
            dinner_time = utils.pad_hour(form.dinner_start.data.hour) + utils.pad_hour(form.dinner_start.data.minute) + utils.pad_hour(form.dinner_end.data.hour) + utils.pad_hour(form.dinner_end.data.minute)
        ...
        if classes:
            ...
            score_params = {'lunch':lunch/100, 'dinner':dinner/100, 'offtime':offtime/100, 'lunch_time':lunch_time, 'dinner_time':dinner_time}
